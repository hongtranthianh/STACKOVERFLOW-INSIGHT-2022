...
def df_for_graph_func(df, col_list, separator, top_n):
    '''
    Get dataframe format ready for networkx graph
    Input:
        df: a dataframe contains columns needed for network graph
        col_list: list of columns in df
        separator: separator to seprate value in each column
        top_n: e.g., number of 7 most commly used technology
    Output:
        Network graph 
    '''
...
def get_node_size_dict_func(df, col, separator):
    '''
    This function would be used inside networkx_graph function
    Input:
        df: equal to renamed_df which would be gotten from df_for_graph_func function
        col: column in df needs to get value count
    Output:
        a dictionary shows key (node) and value (size) based on percentage of value count
    '''
...
def networkx_graph(graph_df, rename_df, title, x_fig=9, y_fig=6, edge_color='#d6d5d5'):
    '''
    Input:
        graph_df: equal to graph_df which would be gotten from df_for_graph_func function
        renamed_df: equal to renamed_df which would be gotten from df_for_graph_func function
        title: title of the graph
        x_fig, y_fig: params for figsize
        edge_color: color of edge
    Output:
        Network graph
    '''
...
def get_networkx_graph(df, col_list, separator, top_n, title, x_fig=9, y_fig=6, edge_color='#d6d5d5'):
    '''
    By passing all params to this function, we can get network graph in a single step
    Input:
        df: a dataframe contains columns needed for network graph
        col_list: list of columns in df
        separator: separator to seprate value in each column
